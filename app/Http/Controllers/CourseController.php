    ...
    public function store(Request $request)
{
    try {
        ...
        foreach ($request->modules as $moduleIndex => $moduleData) {
            ...
            foreach ($moduleData['lessons'] as $lessonIndex => $lessonData) {
                // Verificar y corregir la URL del video antes de crear la lección
                if ($lessonData['type'] === 'video' && isset($lessonData['content'])) {
                    // Verificar si la URL es de YouTube
                    if (strpos($lessonData['content'], 'youtube.com/watch') !== false) {
                        parse_str(parse_url($lessonData['content'], PHP_URL_QUERY), $queryParams);
                        if (isset($queryParams['v'])) {
                            // Convertir la URL de YouTube a formato de embeber
                            $lessonData['content'] = 'https://www.youtube.com/embed/' . $queryParams['v'];
                        }
                    }
                    
                    // Verificar si la URL es de Vimeo
                    elseif (strpos($lessonData['content'], 'vimeo.com') !== false) {
                        $videoId = basename(parse_url($lessonData['content'], PHP_URL_PATH));
                        // Convertir la URL de Vimeo a formato de embeber
                        $lessonData['content'] = 'https://player.vimeo.com/video/' . $videoId;
                    }
                }

                // Crear la lección con los datos corregidos
    ...
    public function show(Course $course)
    {
        ...
        return view('show', compact('course', 'isEnrolled' ));
    }
